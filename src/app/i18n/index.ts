...
const initI18next = async (lng: string, ns: string | string[]) => {
  ...
  await i18nInstance
    ...
    .use(
      resourcesToBackend((language: string, namespase: string) => import(`./locales/${language}/${namespase}.json`))
    )
...
>(
  ...
  const i18nextInstance = await initI18next(lng, Array.isArray(ns) ? ns[0] : ns as string)
  return {
